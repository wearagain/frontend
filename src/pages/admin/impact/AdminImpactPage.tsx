...
import PeriodImpactReceipt from "@/components/admin/impact/PeriodImpactReceipt.tsx";
import ChartSection from "@/components/admin/impact/ChartSection.tsx";
...
const AdminImpactPage = () => {
  ...
  return (
    <StatusHandler isLoading={isLoading} isError={isError} error={error}>
      <div className="flex flex-col h-full bottombar-p mb-[300px]">
        ...
        {/* 필터 및 다운로드 영역 */}
        <div className="main-inner p-5 flex items-center justify-between">
          {activeTab === "period"
            ? (
              <PeriodImpactReceipt
                onCalendarClick={() => setShowPeriodSelector(true)}
                dateRange={dateRange}
        ...
        {/* 요약 카드 */}
        {summaryData && (
          <div className="main-inner pb-4 pr-5">
            <SummaryCard
              title="요약"
              data={
                activeTab === "period"
        ...
        {/* 환경영수증 섹션 */}
        {environmentalData && (
          <div className="main-inner py-4 pr-5">
        ...
        <ChartSection title="주간 지표"/>
        <div className='divider-compact pt-3 mb-5'></div>
        <ChartSection title="월간 지표"/>
